 ...
 */
export function detectBlocks(html, config) {
    // Parse block elements using regex (no DOM available in Node context)
    const blockPattern = new RegExp(`<div[^>]+class=["']([^"']*?)["'][^>]*>`, 'gi');
 ...
 */
function extractBlockInfo(classes, config) {
    if (classes.length === 0)
        return null;
    ...
    if (config.nameStrategy === 'class') {
        ...
        const variations = config.variationStrategy === 'remaining-classes'
            ? meaningful.slice(1)
            : [];
 ...
 */
function buildSignature(name, variations, instance) {
 ...
 */
export function buildBlockSignatures(blockName, variations, textContent) {
...
//# sourceMappingURL=block-detector.js.map