 ...
 */
export function generateHtmlReport(report, outputDir) {
...
function buildHtml(report, outputDir) {
...
function buildPageRow(c, outputDir) {
    ...
    if (c.dom) {
        for (const d of c.dom) {
            if (d.missingInBranch)
                issues.push(`Missing block: ${d.blockName}`);
            if (d.hasHorizontalOverflow)
                issues.push(`Overflow: ${d.blockName}`);
            if (d.headingChanges.before.length > 0)
                issues.push(`Heading changed: ${d.blockName}`);
        }
    }
...
function buildScreenshotsHtml(c, outputDir) {
    const links = [];
    const toRelative = (p) => {
        if (!p)
            return null;
    ...
    if (before)
        links.push(`<a href="${before}" target="_blank">Before</a>`);
    if (after)
        links.push(`<a href="${after}" target="_blank">After</a>`);
    if (diff)
        links.push(`<a href="${diff}" target="_blank">Diff</a>`);
    if (links.length === 0)
        return '—';
 ...
 */
export function saveReports(report, outputDir) {
    if (!existsSync(outputDir)) {
        mkdirSync(outputDir, { recursive: true });
    }
    // JSON report
