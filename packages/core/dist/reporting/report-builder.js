 ...
 */
export function classifyPageStatus(comparison, thresholds) {
    const issues = [];
    let status = 'passed';
    const escalate = (to) => {
    ...
    if (comparison.a11y) {
        if (comparison.a11y.newCriticalCount > 0 && thresholds.failOnNewCriticalAccessibilityIssue) {
            escalate('failed');
            issues.push(`${comparison.a11y.newCriticalCount} new critical accessibility violation(s)`);
        }
    ...
    const summary = issues.length === 0 ? 'No regressions detected' : issues.join('; ');
    return { status, summary };
}
/**
 * Aggregate all page comparison results into a report summary.
 */
export function buildReportSummary(comparisons) {
    const summary = {
        passed: 0,
        warnings: 0,
        failed: 0,
    ...
    for (const c of comparisons) {
        ...
        if (c.dom) {
            if (c.dom.some((d) => d.hasHorizontalOverflow))
                summary.pagesWithOverflow++;
            if (c.dom.some((d) => d.missingInBranch))
                summary.pagesWithMissingBlocks++;
        }
    }
    return summary;
}
