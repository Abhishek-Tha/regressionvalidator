    ...
    for (const blockName of blockNames) {
        ...
        if (imports.length > 0) {
            ...
            for (const imp of imports) {
                const existing = moduleToBlocks.get(imp) ?? new Set();
 ...
 */
export function findTransitivelyAffectedBlocks(graph, changedFiles) {
    const affectedBlocks = new Map();
