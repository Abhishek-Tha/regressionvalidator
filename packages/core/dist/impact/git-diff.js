 ...
 */
export function getChangedFiles(baseRef, headRef, projectRoot) {
    try {
        const output = execSync(`git -C "${projectRoot}" diff --name-only "${baseRef}...${headRef}"`, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'] });
 ...
 */
export function getCurrentCommitSha(projectRoot) {
 ...
 */
export function refExists(ref, projectRoot) {
    try {
 ...
 */
export function groupChangedFilesByBlock(changedFiles) {
