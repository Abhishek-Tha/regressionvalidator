...
import { BlockGuardConfigSchema, BlockGuardConfig } from './schema.js';

/**
 * Load and validate a blockguard.config.yml from the given path.
 * Falls back to sensible defaults via the Zod schema.
 */
export function loadConfig(configPath: string): BlockGuardConfig {
 ...
 */
export function resolveLiveOrigin(config: BlockGuardConfig): string {
 ...
 */
export function resolvePreviewOrigin(config: BlockGuardConfig, branch: string): string {
  const pattern =
    config.site.previewHostPattern ??
