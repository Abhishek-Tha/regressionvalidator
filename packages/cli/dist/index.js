...
import { loadConfig, analyzeImpact, findBlockUsage, loadIndex, saveIndex, createEmptyIndex, getAllBlockNames, hashConfig, generateMarkdownSummary, resolveLiveOrigin, discoverFromQueryIndex, discoverFromSitemap, } from '@blockguard/core';
...
async function cmdIndex() {
    ...
    const index = createEmptyIndex(`${config.site.owner}/${config.site.repo}`, 'manual', configHash);
...
async function cmdUsage() {
    ...
    if (!blockName)
        error('Usage: blockguard usage <block-name> [--variation <var>] [--locale <locale>]');
    const configPath = getConfigPath();
    const config = loadConfig(configPath);
...
async function cmdAnalyze() {
    ...
    if (impact.sharedFilesChanged.length > 0) {
        ...
        for (const f of impact.sharedFilesChanged)
            log(`  • ${f}`);
    }
    log('');
}
async function cmdTest() {
    const blockName = args[1];
    if (!blockName)
        error('Usage: blockguard test <block-name> --base <ref> --head <ref>');
    const baseRef = flags['base'] ?? 'origin/main';
    const headRef = flags['head'] ?? 'HEAD';
