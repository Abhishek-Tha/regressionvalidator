...
import { loadConfig, analyzeImpact, selectRegressionPages, launchBrowser, captureMultiViewport, compareVisuals, classifyPageStatus, buildRegressionReport, saveReports, generateMarkdownSummary, resolveLiveOrigin, resolvePreviewOrigin, discoverFromQueryIndex, discoverFromSitemap, hashConfig, createEmptyIndex, waitForUrl, } from '@blockguard/core';
...
async function run() {
    try {
        ...
        const mode = (core.getInput('mode') || 'representative');
        ...
        const headBranch = context.payload.pull_request?.head?.ref?.replace('refs/heads/', '') ?? 'HEAD';
        ...
        const selectionResult = selectRegressionPages(index.pages, impact.allAffectedBlocks, config.selection);
        core.info(`Selected ${selectionResult.selected.length} / ${selectionResult.totalAffected} pages for testing`);
        core.endGroup();
        ...
        // Check run
        try {
            await upsertCheckRun(octokit, owner, repo, headSha, report.status, report.summary.failed > 0
                ? `${report.summary.failed} regression(s) detected across ${report.testedPages} pages`
                : `${report.testedPages} pages tested — ${report.status}`, report.htmlReportPath);
        }
        catch (err) {
