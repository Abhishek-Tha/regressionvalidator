...
import { analyzeImpact, findBlockUsage, loadIndex, saveIndex, createEmptyIndex, getBlockVariations, hashConfig, selectRegressionPages, launchBrowser, captureMultiViewport, compareVisuals, classifyPageStatus, buildRegressionReport, saveReports, generateMarkdownSummary, discoverFromQueryIndex, discoverFromSitemap, } from '@blockguard/core';
/**
...
// ─── Tool Handlers ────────────────────────────────────────────────────────────
async function handleIndexSiteBlocks(args) {
...
async function handleFindBlockUsage(args) {
...
async function handleAnalyzeCodeChange(args) {
    ...
    return {
        ...
        summary: impact.allAffectedBlocks.length === 0
            ? 'No block changes detected'
            : `${impact.allAffectedBlocks.length} block(s) affected: ${impact.allAffectedBlocks.join(', ')}. Risk: ${impact.risk}.`,
    };
}
async function handleSelectRegressionPages(args) {
    const changedBlocks = args['changedBlocks'] ?? [];
    const mode = (String(args['mode'] ?? 'representative'));
    const maxPages = Number(args['maxPages'] ?? 20);
    const indexPath = args['indexPath']
        ? String(args['indexPath'])
        : '/tmp/blockguard/block-usage-index.json';
    if (!changedBlocks.length)
        throw new Error('changedBlocks is required');
    if (!existsSync(indexPath)) {
        return {
            error: `No usage index found at ${indexPath}. Run index_site_blocks first.`,
        };
    ...
    const selectionConfig = {
        ...
        pagesPerVariation: 2,
        prioritize: ['traffic', 'multiple-instances'],
    };
...
async function handleRunBlockRegression(args) {
    const blockName = String(args['block'] ?? '');
    const baseRef = String(args['baseRef'] ?? 'origin/main');
    const headRef = String(args['headRef'] ?? 'HEAD');
    ...
    const viewportNames = args['viewports'] ?? ['mobile', 'desktop'];
    if (!blockName)
        throw new Error('block is required');
    if (!liveOrigin)
        throw new Error('liveOrigin is required');
    if (!previewOrigin)
        throw new Error('previewOrigin is required');
    // Impact analysis
    const impact = await analyzeImpact({ baseRef, headRef, projectRoot });
        ...
        for (const { page, reasons, affectedBlockNames } of selectionResult.selected) {
            ...
            for (const viewport of viewports) {
                ...
                try {
                    ...
                    const { status, summary } = classifyPageStatus({ pagePath: page.path, baseUrl, branchUrl, viewport: viewport.name,
                        ...
                        diffScreenshot: diffPath }, thresholds);
                    comparisons.push({
                        pagePath: page.path, baseUrl, branchUrl, viewport: viewport.name,
...
async function handleGetRegressionReport(args) {
